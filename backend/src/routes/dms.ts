    ...
    const { rows } = await query(
      `SELECT dc.id, dc.created_at,
              ...
              (SELECT content FROM dm_messages
               WHERE conversation_id = dc.id ORDER BY created_at DESC LIMIT 1) as last_message,
              (SELECT created_at FROM dm_messages
               WHERE conversation_id = dc.id ORDER BY created_at DESC LIMIT 1) as last_message_at
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const { rows: [msg] } = await query(
        ...
        [conversationId, req.user!.id, req.body.content]
      );

      const { rows: [full] } = await query(
        `SELECT dm.*, u.username as sender_username, u.avatar_url as sender_avatar
         FROM dm_messages dm INNER JOIN users u ON u.id = dm.sender_id WHERE dm.id = $1`,
        [msg.id]
      );
